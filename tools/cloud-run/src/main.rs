...
enum Commands {
    ...
    /// Launch experiment on EC2 (spot by default, on-demand fallback)
    Launch {
        ...
        /// Force on-demand (skip spot attempt)
        #[arg(long)]
        on_demand: bool,
    },
    ...
    /// Clean up orphaned resources (expired instances, unused SGs)
    Cleanup,
}
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Cleanup => {
            ...
            eprintln!("Cleaning up orphaned resources in {}...", region);

            // 1. Terminate TTL-expired instances
            eprintln!("\n[1/2] Checking for TTL-expired instances...");
            ...
            // 2. Clean up orphaned security groups
            eprintln!("\n[2/2] Cleaning up orphaned security groups...");
